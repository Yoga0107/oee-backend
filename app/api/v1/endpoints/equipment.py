...
def list_equipment(
    ...
    search:       str | None  = Query(None, description="Cari di semua kolom teks"),
...
def get_stats(current_user: CurrentUser, plant: CurrentPlant):
    """Ringkasan statistik: total, verified, unverified per sistem."""
    db = _db(plant)
...
def verify_bulk(
    ...
    """Verifikasi banyak record sekaligus. Body: {"ids": [1,2,3], "action": "verify"|"unverify"}"""
...
def export_equipment(
    ...
    if is_verified is not None:
        q = q.filter(EquipmentTree.is_verified == is_verified)
    items = q.order_by(
        EquipmentTree.sistem, EquipmentTree.sub_sistem,
        EquipmentTree.unit_mesin, EquipmentTree.bagian_mesin,
        EquipmentTree.spare_part,
    ).all()
    ...
    VERIFIED_FILL  = PatternFill("solid", fgColor="D1FAE5")
    PENDING_FILL   = PatternFill("solid", fgColor="FEF9C3")

    for ri, item in enumerate(items, 2):
        row_vals = [
            item.sistem, item.sub_sistem, item.unit_mesin,
            item.bagian_mesin, item.spare_part, item.spesifikasi,
            item.sku, item.bu,
            "✓ Verified" if item.is_verified else "Pending",
            item.remarks,
        ]
        fill = VERIFIED_FILL if item.is_verified else PENDING_FILL
        for ci, val in enumerate(row_vals, 1):
            ...
            if ci == 9:
                c.fill = fill
                c.font = Font(
                    color="065F46" if item.is_verified else "713F12",
    ...
    IH = [
        ("sistem*", 20), ("sub_sistem", 22), ("unit_mesin", 25),
        ("bagian_mesin", 22), ("spare_part", 25), ("spesifikasi", 35),
        ("sku", 18), ("bu", 10), ("remarks", 25),
    ]
    ...
    # Contoh baris
    example = ["INTAKE", "FINE INTAKE", "Blower Fan - BF 111", "Motor",
               "Motor", "Teco AEEBKBO20020FMB 15 kW 3000 rpm", "", "FF", ""]
...
async def import_equipment(
    ...
    if "Import Template" not in wb.sheetnames:
        raise HTTPException(400, "Sheet 'Import Template' tidak ditemukan. Gunakan template dari Export.")
    ...
    for rn, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        if not sistem:
            errors.append({"row": rn, "errors": ["kolom 'sistem' wajib diisi"]})
            continue
